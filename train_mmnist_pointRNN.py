...
parser.add_argument('--model', type=str, default='PointRNN',
                    help='Model to run on the data (stgcnn, dgcnn, tgcnn, modified_edgecnn) [default: modified_edgecnn]')
parser.add_argument('--log_dir', default='mmnist_pointrnn', help='Log dir [default: stgcnn]')
...
model = MODEL.PointRnn(20).to(device)
...
def train():
    ...
    for data in tqdm(train_loader):
        ...
        encoded = model.encode(data.pos.float())
        decoded = model.decode(data.pos.float(), encoded)
        step_loss, step_ch_loss, step_emd_loss = calc_loss(decoded, labels)
...
def test(loader):
    ...
    for data in tqdm(loader):
        data = data.to(device)
        with torch.no_grad():
            input_x = data.x.reshape([-1, 10, 128, 4])[:, :, :, 1:].float()
            encoded = model.encode(input_x)
            decoded = model.decode(input_x, encoded)
            labels = data.y.reshape([-1, 10, 128, 4])[:, :, :, 1:].float()
            loss, ch_loss, emd_loss = calc_loss(decoded, labels)
...
for epoch in range(1, MAX_EPOCH):
    ...
    print(current_loss)
    print(current_ch_loss)
    print(current_emd_loss)
    loss,_,_ = train()
#     scheduler.step()
    current_loss, current_ch_loss, current_emd_loss = test(test_loader)
